//! Simple thread-safe mutext
//!
//! As the doc of SyncUnsafeCell says, `SyncUnsafeCell::get()` can be used to get `*mut T` from
//! `&SyncUnsafeCell<T>` but we must ensure that the access to the object pointed is unique before
//! dereferencing it.
//!
//! This mutex protects the data with AtomicBool to ensure that the access to the content is unique
//! so taking a mutable reference to it will be safe.
