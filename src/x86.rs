...
// SDM Vo.3: 6.14.2 64-Bit Mode Stack Frame
...
global_asm!(
    ...
    mov rdi, rsp
    // Align the stacfk to 16-bytes boundary
    mov rbp, rsp
    and rsp, -16
    // 2nd parameter: Int#
    mov rsi, rcx
    ...
    match index {
        ...
        14 => {
            ...
            error!(
                ...
                if info.error_code & 0b000_0100 != 0 {
                    "user"
                } else {
                    "supervisor"
                },
...
// PDDRTTT (TTTT: type, R: reserved, D: DPL, P: present)
...
/// # Safety
/// This function modifies the page table as callback does,
/// so anything bad can happen if there are some mistakes.
